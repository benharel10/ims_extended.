...
export default function ProductionPage() {
    ...
    return (
        <div className="animate-fade-in">
            ...
            {/* --- DEFINE ASSEMBLY TAB --- */}
            {activeTab === 'define' && (
                <div className="card full-width-page" style={{ paddingLeft: 0, paddingRight: 0, borderLeft: 'none', borderRight: 'none', borderRadius: 0 }}>
                                    ...
                                    <tbody>
                                        {productionRuns.length === 0 ? (
                                            ...
                                            productionRuns.map(run => {
                                                ...
                                                return (
                                                    <tr key={run.id} className={`expandable-row ${isExpanded ? 'expanded' : ''}`} onClick={() => toggleRow(run.id)} style={{ borderBottom: '1px solid var(--border-color)', cursor: 'pointer' }}>
                                                        <td style={{ padding: '0.75rem' }} data-label="Select" onClick={e => e.stopPropagation()}>
                                                            <input
                                                                type="checkbox"
                                                        ...
                                                        <td style={{ padding: '0.75rem' }} data-label="Type">
                                                            <span className="badge badge-success" style={{ fontSize: '0.75rem' }}>{run.status}</span>
                                                        </td>
                                                        <td style={{ padding: '0.75rem', textAlign: 'right', display: 'flex', justifyContent: 'flex-end', gap: '0.5rem' }} data-label="Actions" onClick={e => e.stopPropagation()}>
                                                            <button
                                                                className="mobile-expand-btn btn btn-sm btn-outline"
                                                                style={{ display: 'none' }}
                                                                onClick={(e) => { e.stopPropagation(); toggleRow(run.id); }}
                                                            >
